...
img_sm = [{d['id']: d.get('image_uris', {}).get('small', 'N/A')} for d in card_dict if d['lang'] == 'en' and d['set_name'] == 'Core Set 2019']
img_normal = [{d['id']: d.get('image_uris', {}).get('normal', 'N/A')} for d in card_dict if d['lang'] == 'en' and d['set_name'] == 'Core Set 2019']
img_lg = [{d['id']: d.get('image_uris', {}).get('large', 'N/A')} for d in card_dict if d['lang'] == 'en' and d['set_name'] == 'Core Set 2019']
# img_lg_test = img_lg[1144:1180]
img_png = [{d['id']: d.get('image_uris', {}).get('png', 'N/A')} for d in card_dict if d['lang'] == 'en' and d['set_name'] == 'Core Set 2019']
img_art_crop = [{d['id']: d.get('image_uris', {}).get('art_crop', 'N/A')} for d in card_dict if d['lang'] == 'en' and d['set_name'] == 'Core Set 2019']
img_border_crop = [{d['id']: d.get('image_uris', {}).get('border_crop', 'N/A')} for d in card_dict if d['lang'] == 'en' and d['set_name'] == 'Core Set 2019']
...
for dicts in img_lg:  # iterate over the list of dicts
    for filename, url in dicts.items():  # for each dict, iterate over each key-value pair
        fullfilename = os.path.join('lg_img_Core_2019/', filename + '_lg.jpg')  # location for storage and add suffix to file name
        try:
...
for dicts in img_art_crop:  # iterate over the list of dicts
    for filename, url in dicts.items():  # for each dict, iterate over each key-value pair
        fullfilename = os.path.join('art_crop_img_Core_2019/', filename + '_art_crop.jpg')  # location for storage and add suffix to file name
        try:
            urllib.request.urlretrieve(url, fullfilename)  # try to grab the image
        except ValueError: # if there is no URL/image then fail silently
            pass